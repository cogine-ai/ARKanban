...
- 上述方向依赖一个假设：`chat.history` 的 `offset` 从最新一条往回数（协议文档如此，未在真机逐页核对）。若真机相反，表现是活跃会话只反复读到最旧一页、尾部永不更新；改 `syncSession` 的 `tail` 分支即可，其余逻辑不受影响。`transcript-sync.test.ts` 里的假 Gateway 已按该假设实现倒序翻页。
