...
删会话是三条只有一起做才成立的删除（正文、会话行、指向它的 Activity 引用），因此在同一个 `BEGIN IMMEDIATE` 事务内完成：分开执行时中途崩溃会留下没有代码能解释的状态——正文没了却仍以「已归档、可搜索」示人，或者 Activity 指向一个不存在的会话键。**待删集合也要在写锁之内挑**：先在锁外查出 key 列表、再在锁内按同一条件删会话，两者可能不是同一批——期间被别的写者归档的会话会被条件捞走，而它的正文从来不在那份列表里，于是文本留了下来，却没有任何会话能把它读出来。
