...
4. ~~**audit 采集**完全没实现~~ 已接（§3 有真机形状与复核）：`audit_events` 表（schema v6）按 `eventId` 幂等，同步是「尾读到已知水位线 + 往下回填」，水位线只在读到已知记录后前移，并识别 Gateway 侧裁剪导致的序号回退。审计结论成为派生信号的**首选**工具证据（算法版本 3 → 4），`agent_run` 的结论只在本机 Activity 行一条都没分类出结论时补位。诊断端点新增 `audit`（同步结果 + 行数 + 序号区间）。
