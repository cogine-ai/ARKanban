...
- 终态结论同样先看审计轨的 `agent_run` 行，但只在本机 Activity 行**一条都没分类出结论**时才用它。Activity 是采集器持续维护的会话状态，审计轨是补一个我们本来没有的答案，不是拿来盖掉已有答案的——顺序反过来会让一次运行结束后又重启的会话被更旧的那次结论定性。它补的正是「运行确实结束过、但没有任何事件说它怎么结束」这一类：此前这类会话只能靠静默时长判，六小时后一律记成 `abandoned`。
